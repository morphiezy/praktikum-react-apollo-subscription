...
const TodoList = () => {
  ...
  const [isResubscribe , setResubscribe] = useState(true);
  ...
  const { data, loading } = useSubscription(SUBSCRIPTION_TODO, {onSubscriptionData : ()=> setResubscribe(false)});
  ...
  const findTodoByTitle = () => {
    ...
    const shallowCopyData = [...data.todolist];
    const pattern = new RegExp(title,"i");
    const filterData = shallowCopyData.filter(item => pattern.test(item.title) && item.id_user === selectValue )
  ...
  return (
    <>
      <div className="container">
        ...
        {
          loading || isResubscribe ? ( <Loading /> ) : isInitialQuery ? (
